...
export default function DebtBalanceReviewView({
  ...
  if (storageState.stepIndex === debtBalanceTotalQuestions) {
    return (
      <main className="debt-balance-view flex min-w-0 min-h-0 flex-1 flex-col overflow-hidden bg-white rounded-[20px] shadow-sm border border-gray-200 animate-fade-up delay-100">
              ...
              <CardHeader className="flex flex-col gap-3 px-[18px] pb-0 pt-[18px] sm:flex-row sm:items-center sm:justify-between">
                <div>
                  ...
                  <CardDescription className="text-[12px] text-gray-600 mt-1">
                    Review your selected responses for each scenario to understand your debt pressure analysis.
                  </CardDescription>
                </div>
              </CardHeader>

              <CardContent className="space-y-4 px-[18px] pb-[20px] pt-[16px]">
                {debtBalanceQuestions.map((q, idx) => {
                    ...
                    >
                      ...
                      <div className="grid gap-2.5">
                        {q.options.map((opt, optIdx) => {
                          ...
                          const isSelected = Boolean(selectedOptionId && (selectedOptionId === (opt as any).id || selectedOptionId === opt.label));
